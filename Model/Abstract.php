<?php
abstract class Crud_Model_Abstract implements Crud_Model_Interface {
    /**
    ...
    public function save(array $data, Zend_Db_Table_Row_Abstract $item = null) {
        ...
        foreach($data as $k => $v) {
            if(!in_array($k, $cols)) throw new Crud_Model_Exception('Column ' . $k . 'does not exist');
            $item->{$k} = $v;
        }
